...
#Categorical columns
cat_cols = ['Gender', 'Country', 'University Degree', 'Profession','Hair Color']
...
#'regressor__colsample_bytree':[i/10.0 for i in range(6,11)]
        ...
        'regressor__iterations':[500, 1000],#[250,100,500,1000],
        'regressor__learning_rate':[0.01,0.1,0.2,0.3], 
         'regressor__l2_leaf_reg':[3,1,5,10,100],
         # 'regressor__border_count':[200,250],
...
gridsearch = GridSearchCV(ml_pipe,parameters, verbose=0).fit(X_train, y_train)
...
#predict_X['Profession'] = label.fit_transform(predict_X['Profession'].astype('str'))
